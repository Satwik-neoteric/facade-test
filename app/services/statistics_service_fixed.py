...
class StatisticsService:    
    ...
    async def get_all_statistics(self, batch_id: Optional[str] = None) -> Optional[StatisticsResponse]:
        """
        Get all statistics for either all batches or a specific batch if batch_id is provided.
        Returns a StatisticsResponse object with empty lists if no data is found.
        Returns None if there's an error (not a data condition but a system error).
        """
        try:
            ...
            try:
                ...
                logger.error(f"Failed to initialize CosmosDB service: {init_error}", exc_info=True)
            ...
            return StatisticsResponse(
                ...
                review_stats=[],
                user_comparisons=[]
            )  # Return empty stats instead of None for better error handling
